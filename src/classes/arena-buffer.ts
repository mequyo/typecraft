...
export class ArenaBuffer {
   ...
   */
  constructor(device: GPUDevice, sizeBytes: number, usage: GPUBufferUsageFlags, regions: number[], alignment: number = 16) {
    if ((usage & GPUBufferUsage.COPY_DST) === 0) throw new Error("Usage must include GPUBufferUsage.COPY_DST");
    ...
    for (let i = 0; i < regions.length; i++) {
      ...
      this.regions[i] = { start: offset, size: regionSize, blocks: [{ start: 0, size: regionSize }] };
      offset += regionSize;
    }
  }
  ...
  public write(region: number, data: ArrayBufferView): Allocation {
    ...
    this.device.queue.writeBuffer(this.buffer, alloc.start, data.buffer, data.byteOffset, data.byteLength);
  ...
  public alloc(region: number, sizeBytes: number): Allocation {
    if (region < 0 || region >= this.regions.length) throw new Error(`Invalid region ${region}`);
    ...
    console.warn(`ArenaBuffer.alloc failed for region ${region}: requested ${bytes} bytes, region capacity ${reg.size}`, reg.blocks.slice(0, 20));
    throw new Error(`ArenaBuffer region ${region} out of space (requested ${bytes} bytes, region capacity ${reg.size})`);
  }
  ...
  public free(alloc: Allocation) {
    ...
    const block: FreeBlock = { start: alloc.start - reg.start, size: alloc.size };
   ...
   * Debug info
   */
  public stats(): { capacityBytes: number, freeBytes: number, usedBytes: number } {
    let freeBytes = 0;
    let freeBlocks = 0;

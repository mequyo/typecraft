  ...
  new RenderPipeline({
    ...
    draw: (pass, self, state) => {
      ...
      const mesh = new Float32Array(
        state.world.damaged.size * faces * FLOATS_PER_VERTEX * triangles,
      );
      ...
      for (let i = 0; i < values.length; i++) {
        ...
        const STAGE =
          DESTROY_STAGE_TEXTURES[
            Math.floor(
              (value.damage / value.hardness) * DESTROY_STAGE_TEXTURES.length,
            )
          ];

        offset += cube.write_back_face(
          mesh,
          offset,
          value.position[0],
          value.position[1],

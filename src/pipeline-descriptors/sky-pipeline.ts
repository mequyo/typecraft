...
export const SKY_PIPELINE = (device: GPUDevice): RenderPipeline => new RenderPipeline({
  device,
  ...
  groups: [
    [{
      ...
      update: (state, buffer) => buffer.write(mat4.multiply(mat4.invert(state.player.projection), mat4.invert(state.player.view))),
    }],
  ],
